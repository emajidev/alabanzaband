...
class ContentItem extends React.Component {
  ...
  async statusLikeInSongs() {
    try {
      const postId = this.props.navigation.state.params.item.id;
      const reference = await db.ref("songs/" + postId);
      const comments = await db.ref("songs/" + postId + "/comment");
      reference.on("value", (snapshot) => {
        let info = snapshot.val();
        if (info != null) {
          this.setState({ infoSong: info });
        }
        comments.on("value", (snap) => {
          const comt = snap.val();
          if (comt != null || comt != undefined) {
            let nComment = Object.values(comt);
            this.setState({ nComment: nComment.length });
          }
        });
      });
    } catch (e) {
  ...
  pushLike(status) {
    this.onPostLike(this.props.navigation.state.params.item.id, status).then(
      (transaction) => {
  ...
  componentWillUnmount() {
    this.onPostVisit(this.props.navigation.state.params.item.id).then(
      (transaction) => {
        ...
        >
          ...
          <View>
            <Text style={styles.title}>
              Titulo: <Text style={{}}>{item.name}</Text>{" "}
            </Text>
            <Text style={styles.subtitle}>
              Categoria: <Text style={{}}>{item.category}</Text>
            </Text>
          </View>
        </View>
        <View
          ...
          >
            <View style={{ flexDirection: "row" }}>
              {this.state.statusLike ? (
                <IconLike name="heart" color={"#f45"} size={25} />
              ) : (
                <IconLike name="hearto" color={"#A0A0A0"} size={25} />
              )}

              <Text style={{ color: "#A0A0A0" }}>
                {" "}
                {this.state.infoSong.likes} Me gusta
              </Text>
            </View>
          </TouchableOpacity>
          <TouchableOpacity style={styles.btn_nav} onPress={() => {}}>
            <View style={{ flexDirection: "row" }}>
              <IconView name="eye" color={"#A0A0A0"} size={25} />
              <Text style={{ color: "#A0A0A0" }}>
                {" "}
                {this.state.infoSong.visits} Vistos
              </Text>
            </View>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.btn_nav}
            onPress={() => {
              this.props.navigation.navigate("CommentViewer", {
                props: this.state.infoSong,
              });
            }}
          >
            <View style={{ flexDirection: "row" }}>
              <IconComment name="comment-outline" color={"#A0A0A0"} size={25} />
              <Text style={{ color: "#A0A0A0" }}>
                {this.state.nComment} Comentarios
              </Text>
            </View>
          </TouchableOpacity>
        </View>
